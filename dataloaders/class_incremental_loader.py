...
class IncrementalLoader:
    ...
    def __init__(self, opt, shuffle=True, seed=1):
        ...
        # fills in the self.data_train, etc..    
        self._setup_data(
            datasets,
    ...
    # returns 2,250 data pts (5 classes) for training from the self.data_train containing 90,000
    def new_task(self, memory=None):
        # run out of tasks??
        if self._current_task >= len(self.increments):
            raise Exception("No more tasks.")

        # self.increments = args.increment = [5, 5, 5, 5, 5, 5, 5,...]
        # self._current_task = 0,1,2,3,4...
        min_class = sum(self.increments[:self._current_task])
        max_class = sum(self.increments[:self._current_task + 1])

        # print(self.increments, min_class, max_class) # = [5,5,5..], 0, 5

        # select some (2,250 data pts) for training
        # print(len(self.data_train)) # = [['sdfsd.jpeg','0'],..,['3423eds.jpeg','0']], length = 90,000
        # print(set(self.targets_train)) # = [153, 153, ... , 17, 17, 17], length=90,000, uniq={0,1,2,...,198,199}
        # print('going in')
        # print('train:', min_class, max_class) # = 0, 5
        x_train, y_train = self._select(
            self.data_train, self.targets_train, low_range=min_class, high_range=max_class
        )

        # print('length of x_train:',len(x_train)) # = [['sdfsd.jpeg','0'],..,['3423eds.jpeg','0']], length=2,250 => 1250 yay! => 25

        # select some (250) for validation
        # low_range = 0
        x_val, y_val = self._select(
            self.data_val, self.targets_val, low_range=min_class, high_range=max_class
        )

        # print('len of val', len(x_val)) # 25

        # select some (250) for testing
        # low_range = 0
        x_test, y_test = self._select(self.data_test, self.targets_test, high_range=max_class)

        # add some more from buffer if it exists
        # this doesn't seem to be used much bcz the print statement rarely appears.
    ...
    # select out 250 data pts (x and y)
    def _select(self, x, y, low_range=0, high_range=0):
        # indexes where target between low_range and high_range
        idxes = np.where(np.logical_and(y >= low_range, y < high_range))[0]
        # print('length of x_idx',len(x[idxes])) # = 2250, 250, 250
        # print(idxes)
    ...
    # fills in self.data_train, self.data_test, etc..
    def _setup_data(self, datasets, class_order_type=False, seed=1, increment=10, validation_split=0.):
        ...
        # print('dsfsd',datasets, len(datasets)) = [<class 'dataloaders.idataset.iImgnet'>] 1
        for dataset in datasets:
            # for tinyimagenet
            # print(self._opt)
            '''
            Namespace(alpha_init=0.1, arch='pc_cnn', batch_size=10, batches_per_example=1, beta=1.0, bgd_optimizer='bgd',
            calc_test_accuracy=True, cifar_batches=5, class_order='random', classes_per_it=4, cuda=False, 
            data_path='data/tiny-imagenet-200/', dataset='tinyimagenet', expt_name='lamaml', fisher_gamma=0.95, gamma=1.0, glances=2, 
            grad_clip_norm=1.0, increment=5, iterations=5000, learn_lr=True, loader='class_incremental_loader', log_dir='logs/', 
            log_every=3125, lr=0.001, mean_eta=1, memories=400, memory_strength=0, model='lamaml_cifar', n_epochs=1, n_hiddens=100, 
            n_layers=2, n_memories=0, opt_lr=0.4, opt_wt=0.1, optimizer_params='{}', replay_batch_size=10.0, s=1, samples_per_task=-1, 
            second_order=True, seed=0, shuffle_tasks=False, std_init=0.05, steps_per_sample=1, sync_update=False, test_batch_size=100000, 
            tf_dir='', train_mc_iters=5, use_old_task_memory=False, validation=0.1, workers=3, xav_init=False)
            '''
            if(self._opt.dataset == 'tinyimagenet'):
                # store root path
                root_path = self._opt.data_path

                # get the training and test set images from the folders
                train_dataset = dataset.base_dataset(root_path + 'train/')
                test_dataset = dataset.base_dataset(root_path + 'val/')

                # print(train_dataset) # Dataset ImageFolder Number of datapoints: 100000 Root location: data/tiny-imagenet-200/train/
                # print(len(train_dataset.samples)) # [('asd.jpeg', 192),()..], length=100,000

                train_dataset.data = train_dataset.samples
                test_dataset.data = test_dataset.samples

                # store the data samples into the x_train, y_train lists
                x_train, y_train = train_dataset.data, np.array(train_dataset.targets)

                # split into val train (90k) and (10k) sets controlled by validation_split
                ...
                # order = [1,2,3,...,200]
                # print(len(np.unique(y_train)), len(y_train)) # 100, 90,000
            ...
            # this is for cifar dataset i guess
            else:
                root_path =  self._opt.data_path
    ...
    def _list_split_per_class(x, y, validation_split=0.):
        ...
        for class_id in np.unique(y):
            ...
            nb_val_elts2 = int(class_indexes.shape[0] * 0.99) # pretend as if val is taking up half

            val_indexes = class_indexes[:nb_val_elts]

            # train_indexes = class_indexes[nb_val_elts:]
            train_indexes = class_indexes[nb_val_elts2:] # fraudulent train_indexes, lol
